...
if KTSH_TESTING:
    ...
    TDCLIM = {
        ...
        "doubleState2D_1"         : [1117, 1717]
    }

    DS, TS, DS2 = ["tully1D_1", "tully1D_1_lorentz"], ["tripleState1D_4", "tripleState1D_4_lorentz"], ["doubleState2D_1"]
    ...
    for potential in DS2:
        ...
        qd_input = {
            "quantum_dynamics" : {
                ...
                "grid" : {
                    "limits" : [-10, 10], "points" : 256
                },

                "initial_conditions" : {
                    "position" : [0, -5], "momentum" : [0, 0], "gamma" : 2, "state" : IS[potential][0], "mass" : 1
                },
        ...
        cd_input = {
            "classical_dynamics" : {
                ...
                "initial_conditions" : {
                    "position_mean" : qd_input["quantum_dynamics"]["initial_conditions"]["position"], "position_std"  : [0.5, 0.5],
                    "momentum_mean" : qd_input["quantum_dynamics"]["initial_conditions"]["momentum"], "momentum_std"  : [1.0, 1.0],
                    "state" : [int(i == IS[potential][1]) for i in range(NS[potential])],
                    "mass" : [1, 1]
                },
    ...
    os.system(f'python python/lines.py KTSH_TESTING_TDC_{DS2[0]}_FSSH.mat:1 KTSH_TESTING_TDC_{DS2[0]}_KTSH.mat:1 KTSH_TESTING_POPULATION_{DS2[0]}_EXACT.mat:1 KTSH_TESTING_POPULATION_MEAN_{DS2[0]}_FSSH.mat:1 KTSH_TESTING_POPULATION_MEAN_{DS2[0]}_KTSH.mat:1 KTSH_TESTING_POPULATION_MEAN_{DS2[0]}_LZSH.mat:1 --dpi 256 --figsize 4 12 --legends every "T{dollar}_{{0,1}}{dollar} (HST)" "T{dollar}_{{0,1}}{dollar} (BA)" "S{dollar}_1{dollar} (EXACT)" "S{dollar}_1{dollar} (FSSH)" "S{dollar}_1{dollar} ({dollar}{esc}kappa{dollar}TSH)" "S{dollar}_1{dollar} (LZSH)" --subplots 121 121 122 122 122 122 --xlabel "Coordinate (a.u.)" "Time (a.u.)" --ylabel "TDC (??)" "Excited State Population" --output MODEL_DS2.png')
