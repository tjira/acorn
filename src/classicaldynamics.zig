...
pub fn run(comptime T: type, opt: ClassicalDynamicsOptions(T), print: bool, allocator: std.mem.Allocator) !ClassicalDynamicsOutput(T) {
...

    {
        ...
        for (0..opt.trajectories) |i| {
            ...
            for (0..r.rows) |j| (try r.ptr(j)).* = opt.initial_conditions.position_mean[j] + opt.initial_conditions.position_std[j] * rand_traj.floatNorm(T);
            for (0..p.rows) |j| (try p.ptr(j)).* = opt.initial_conditions.momentum_mean[j] + opt.initial_conditions.momentum_std[j] * rand_traj.floatNorm(T);

            if (fssh) {C.fill(Complex(T).init(0, 0)); (try C.ptr(opt.initial_conditions.state)).* = Complex(T).init(1, 0);   }
            ...
            for (0..opt.iterations) |j| {
                ...
                for (0..r.rows) |k| {
                    ...
                    (try r.ptr(k)).* += 1 * opt.derivative_step; try mpt.eval(T, &U, opt.potential, r); if (opt.adiabatic) {mat.eigh(T, &UA, &UC, U, &T1, &T2); @memcpy(U.data, UA.data);} const Up = U.at(s, s);
                    (try r.ptr(k)).* -= 2 * opt.derivative_step; try mpt.eval(T, &U, opt.potential, r); if (opt.adiabatic) {mat.eigh(T, &UA, &UC, U, &T1, &T2); @memcpy(U.data, UA.data);} const Um = U.at(s, s);

                    (try a.ptr(k)).* = -0.5 * (Up - Um) / opt.derivative_step / opt.initial_conditions.mass;
                    (try v.ptr(k)).* += 0.5 * (try a.at(k) + try ap.at(k)) * opt.time_step;
                    (try r.ptr(k)).* += (try v.at(k) + 0.5 * try a.at(k) * opt.time_step) * opt.time_step + opt.derivative_step;
                }
                ...
                if (sp != s) for (0..v.rows) |k| {
                    (try v.ptr(k)).* *= std.math.sqrt((Ekin - U.at(s, s) + U.at(sp, sp)) / Ekin);
                };
                ...
                if (opt.write.position_mean                 != null) for (0..r.rows) |k| {position.ptr(j, k).* += try r.at(k);}                                      ;
                if (opt.write.momentum_mean                 != null) for (0..v.rows) |k| {momentum.ptr(j, k).* += try v.at(k) * opt.initial_conditions.mass;}        ;
                if (opt.write.time_derivative_coupling_mean != null) for (0..tdc.cols) |k| {tdc.ptr(j, k).* += TDC.data[k];}                                         ;
                if (opt.write.fssh_coefficient_mean         != null) for (0..C.rows) |k| {coef.ptr(j, k).* += (try C.at(k)).magnitude() * (try C.at(k)).magnitude();};

                if (j == opt.iterations - 1) {
                    (try output.pop.ptr(s)).* += 1;
                }
    ...
    for (0..output.pop.rows) |i| (try output.pop.ptr(i)).* /= asfloat(T, opt.trajectories);

    for (0..nstate) |i| {
        if (print) {try std.io.getStdOut().writer().print("{s}FINAL POPULATION OF STATE {d:2}: {d:.6}\n", .{if (i == 0) "\n" else "", i, try output.pop.at(i)});}
    }
...
fn fewestSwitches(comptime T: type, C: *Vector(Complex(T)), opt: ClassicalDynamicsOptions(T).FewestSwitches, U: Matrix(T), TDC: Matrix(T), s: u32, time_step: T, Ekin: T, rand: std.Random, K1: @TypeOf(C), K2: @TypeOf(C), K3: @TypeOf(C), K4: @TypeOf(C)) !u32 {
    ...
    const Function = struct { fn get (K: *Vector(Complex(T)), FC: Vector(Complex(T)), FU: Matrix(T), FTDC: Matrix(T)) !void {
        for (0..FC.rows) |i| {(try K.ptr(i)).* = (try FC.at(i)).mul(Complex(T).init(FU.at(i, i), 0)).mulbyi().neg();} for (0..FC.rows) |i| for (0..FC.rows) |j| {
            (try K.ptr(i)).* = (try K.at(i)).sub((try FC.at(j)).mul(Complex(T).init(FTDC.at(i, j), 0)));
        };
    }};
    ...
    for (0..opt.quantum_substep) |_| {
        ...
        try Function.get(K1, C.*, U, TDC);

        for (0..C.rows) |j| {
            (try C.ptr(j)).* = (try C.at(j)).add((try K1.at(j)).mul(Complex(T).init(time_step / 2 / iters, 0)));
        }

        try Function.get(K2, C.*, U, TDC);

        for (0..C.rows) |j| {
            (try C.ptr(j)).* = (try C.at(j)).sub((try K1.at(j)).mul(Complex(T).init(time_step / 2 / iters, 0)));
            (try C.ptr(j)).* = (try C.at(j)).add((try K2.at(j)).mul(Complex(T).init(time_step / 2 / iters, 0)));
        }

        try Function.get(K3, C.*, U, TDC);

        for (0..C.rows) |j| {
            (try C.ptr(j)).* = (try C.at(j)).sub((try K2.at(j)).mul(Complex(T).init(time_step / 2 / iters, 0)));
            (try C.ptr(j)).* = (try C.at(j)).add((try K3.at(j)).mul(Complex(T).init(time_step / 1 / iters, 0)));
        }

        try Function.get(K4, C.*, U, TDC);

        for (0..C.rows) |j| {
            (try C.ptr(j)).* = (try C.at(j)).sub((try K3.at(j)).mul(Complex(T).init(time_step / iters, 0)));
        }

        for (0..C.rows) |j| {
            (try C.ptr(j)).* = (try C.at(j)).add((try K1.at(j)).add((try K2.at(j)).mul(Complex(T).init(2, 0))).add((try K3.at(j)).mul(Complex(T).init(2, 0))).add((try K4.at(j))).mul(Complex(T).init(time_step / 6 / iters, 0)));
        }

        const rn = rand.float(T); var pm: T = 0; for (0..C.rows) |j| if (j != ns) {
            const p = 2 * TDC.at(ns, j) * (try C.at(j)).mul((try C.at(ns)).conjugate()).re / std.math.pow(T, (try C.at(ns)).magnitude(), 2) * time_step / iters; if (rn < p and p > pm) {ns = @intCast(j); pm = p;}
        };

        for (0..C.rows) |j| if (j != ns) {
            (try C.ptr(j)).* = (try C.at(j)).mul(Complex(T).init(std.math.exp(-0.5 * @abs(U.at(j, j) - U.at(ns, ns)) * time_step / iters / (1 + alpha / Ekin)), 0));
        };

        var sumc: T = 0; for (0..C.rows) |j| if (j != ns) {sumc += (try C.at(j)).magnitude() * (try C.at(j)).magnitude();};

        if ((try C.at(ns)).magnitude() > 0) {
            (try C.ptr(ns)).* = (try C.at(ns)).mul(Complex(T).init(std.math.sqrt((1 - sumc) / (try C.at(ns)).magnitude() / (try C.at(ns)).magnitude()), 0));
        }
    }
...
fn initialBlochVector(comptime T: type, S: *Vector(T), opt: ClassicalDynamicsOptions(T).SpinMapping, s: u32, rand: std.Random, allocator: std.mem.Allocator) !void {
    ...
    (try S.ptr(0)).* = std.math.sin(theta) * std.math.cos(phi); (try S.ptr(1)).* = std.math.sin(theta) * std.math.sin(phi); (try S.ptr(2)).* = std.math.cos(theta);
    ...
    if (opt.sample_bloch_vector) {
        ...
        (try x.ptr(0)).* = std.math.cos(theta) * std.math.cos(phi); (try x.ptr(1)).* = std.math.cos(theta) * std.math.sin(phi); (try x.ptr(2)).* = -std.math.sin(theta);
        (try y.ptr(0)).* = -std.math.sin(phi);                      (try y.ptr(1)).* = std.math.cos(phi);                       (try y.ptr(2)).* = 0;

        for (0..S.rows) |j| {
            (try S.ptr(j)).* = try S.at(j) / std.math.sqrt(3) + (try x.at(j) * std.math.cos(xi) + try y.at(j) * std.math.sin(xi)) * std.math.sqrt2 / std.math.sqrt(3);
        }
    }
}
...
fn spinMapping(comptime T: type, S: *Vector(T), opt: ClassicalDynamicsOptions(T).SpinMapping, U: Matrix(T), TDC: Matrix(T), s: u32, time_step: T, rand: std.Random) !u32 {
    ...
    const FunctionX = struct { fn get (FS: Vector(T), FU: Matrix(T), FTDC: Matrix(T)) !T {
        return (FU.at(1, 1) - FU.at(0, 0)) * try FS.at(1) - 2 * FTDC.at(0, 1) * try FS.at(2);
    }};

    const FunctionY = struct { fn get (FS: Vector(T), FU: Matrix(T)                 ) !T {
        return -(FU.at(1, 1) - FU.at(0, 0)) * try FS.at(0);
    }};

    const FunctionZ = struct { fn get (FS: Vector(T),                FTDC: Matrix(T)) !T {
        return 2 * FTDC.at(0, 1) * try FS.at(0);
    }};

    for (0..opt.quantum_substep) |_| {

        const kx1 = try FunctionX.get(S.*, U, TDC);
        const ky1 = try FunctionY.get(S.*, U,    );
        const kz1 = try FunctionZ.get(S.*,    TDC);

        (try S.ptr(0)).* += 0.5 * kx1 * time_step / iters;
        (try S.ptr(1)).* += 0.5 * ky1 * time_step / iters;
        (try S.ptr(2)).* += 0.5 * kz1 * time_step / iters;

        const kx2 = try FunctionX.get(S.*, U, TDC);
        const ky2 = try FunctionY.get(S.*, U     );
        const kz2 = try FunctionZ.get(S.*,    TDC);

        (try S.ptr(0)).* += 0.5 * (kx2 - kx1) * time_step / iters;
        (try S.ptr(1)).* += 0.5 * (ky2 - ky1) * time_step / iters;
        (try S.ptr(2)).* += 0.5 * (kz2 - kz1) * time_step / iters;

        const kx3 = try FunctionX.get(S.*, U, TDC);
        const ky3 = try FunctionY.get(S.*, U     );
        const kz3 = try FunctionZ.get(S.*,    TDC);

        (try S.ptr(0)).* += 0.5 * (2 * kx3 - kx2) * time_step / iters;
        (try S.ptr(1)).* += 0.5 * (2 * ky3 - ky2) * time_step / iters;
        (try S.ptr(2)).* += 0.5 * (2 * kz3 - kz2) * time_step / iters;

        const kx4 = try FunctionX.get(S.*, U, TDC);
        const ky4 = try FunctionY.get(S.*, U     );
        const kz4 = try FunctionZ.get(S.*,    TDC);

        (try S.ptr(0)).* -= kx3 * time_step / iters;
        (try S.ptr(1)).* -= ky3 * time_step / iters;
        (try S.ptr(2)).* -= kz3 * time_step / iters;

        (try S.ptr(0)).* += time_step / iters / 6 * (kx1 + 2 * kx2 + 2 * kx3 + kx4);
        (try S.ptr(1)).* += time_step / iters / 6 * (ky1 + 2 * ky2 + 2 * ky3 + ky4);
        (try S.ptr(2)).* += time_step / iters / 6 * (kz1 + 2 * kz2 + 2 * kz3 + kz4);

        const rn = rand.float(T); if (!opt.sample_bloch_vector) {
            if (s == sn and s == 0 and rn <  try FunctionZ.get(S.*, TDC) / (1 - try S.at(2)) * time_step / iters) sn = 1;
            if (s == sn and s == 1 and rn < -try FunctionZ.get(S.*, TDC) / (1 + try S.at(2)) * time_step / iters) sn = 0;
        }
    }

    if (opt.sample_bloch_vector) {return if (try S.at(2) > 0) 1 else 0;} else return sn;
}
...
fn printIteration(comptime T: type, i: u32, j: u32, Ekin: T, Epot: T, Etot: T, s: u32, r: Vector(T), v: Vector(T), C: Vector(Complex(T)), S: Vector(T), mass: T, fssh: bool, mash: bool) !void {
    ...
    for (0..r.rows) |k| {
        try std.io.getStdOut().writer().print("{s}{d:9.4}", .{if (k == 0) "" else ", ", try r.at(k)});
    }

    try std.io.getStdOut().writer().print("] [", .{});

    for (0..v.rows) |k| {
        try std.io.getStdOut().writer().print("{s}{d:9.4}", .{if (k == 0) "" else ", ", try v.at(k) * mass});
    }

    if (fssh or mash) try std.io.getStdOut().writer().print("] [", .{});

    if (fssh) for (0..C.rows) |k| {
        try std.io.getStdOut().writer().print("{s}{d:9.4}", .{if (k == 0) "" else ", ", (try C.at(k)).magnitude() * (try C.at(k)).magnitude()});
    };

    if (mash) for (0..S.rows) |k| {
        try std.io.getStdOut().writer().print("{s}{d:9.4}", .{if (k == 0) "" else ", ", try S.at(k)});
    };
