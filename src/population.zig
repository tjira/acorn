...
pub fn mulliken(comptime T: type, system: System(T), basis: std.ArrayList(T), S_AO: Matrix(T), D_MO: Matrix(T), allocator: std.mem.Allocator) !Vector(T) {
    ...
    for (0..S_AO.rows) |i| for (0..S_AO.cols) |j| {
        m.ptr(bf2atom.items[i]).* -= D_MO.at(j, i) * S_AO.at(j, i);
    };
