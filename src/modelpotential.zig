...
pub fn harmonic1D_1(comptime T: type, U: *Matrix(T), r: Vector(T)) !void {
    U.ptr(0, 0).* = 0.5 * try r.at(0) * try r.at(0);
}

pub fn harmonic2D_1(comptime T: type, U: *Matrix(T), r: Vector(T)) !void {
    U.ptr(0, 0).* = 0.5 * (try r.at(0) * try r.at(0) + try r.at(1) * try r.at(1));
}

pub fn doubleState1D_1(comptime T: type, U: *Matrix(T), r: Vector(T)) !void {
    U.ptr(0, 0).* = 0.001 * try r.at(0);
    U.ptr(0, 1).* = 0.001 * std.math.exp(-0.05 * try r.at(0) * try r.at(0));

    U.ptr(1, 0).* = U.at(0, 1);
    U.ptr(1, 1).* = -0.001 * try r.at(0);
}

pub fn doubleState1D_2(comptime T: type, U: *Matrix(T), r: Vector(T)) !void {
    U.ptr(0, 0).* = 0.01 * std.math.tanh(0.6 * try r.at(0));
    U.ptr(0, 1).* = 0.001 * std.math.exp(-try r.at(0) * try r.at(0));

    U.ptr(1, 0).* = U.at(0, 1);
    U.ptr(1, 1).* = -0.01 * std.math.tanh(0.6 * try r.at(0));
}

pub fn tripleState1D_1(comptime T: type, U: *Matrix(T), r: Vector(T)) !void {
    U.ptr(0, 0).* = 0.001 * try r.at(0);
    U.ptr(0, 1).* = 0.001 * std.math.exp(-0.01 * try r.at(0) * try r.at(0));
    U.ptr(0, 2).* = 0.002 * std.math.exp(-0.01 * try r.at(0) * try r.at(0));

    U.ptr(1, 0).* = U.at(0, 1);
    U.ptr(1, 1).* = 0;
    U.ptr(1, 2).* = 0.001 * std.math.exp(-0.01 * try r.at(0) * try r.at(0));

    U.ptr(2, 0).* = U.at(0, 2);
    U.ptr(2, 1).* = U.at(1, 2);
    U.ptr(2, 2).* = -0.001 * try r.at(0);
}

pub fn tripleState1D_2(comptime T: type, U: *Matrix(T), r: Vector(T)) !void {
    U.ptr(0, 0).* = 0.01 * std.math.tanh(0.5 * try r.at(0));
    U.ptr(0, 1).* = 0.001 * std.math.exp(-try r.at(0) * try r.at(0));
    U.ptr(0, 2).* = 0.002 * std.math.exp(-try r.at(0) * try r.at(0));

    U.ptr(1, 0).* = U.at(0, 1);
    U.ptr(1, 1).* = 0;
    U.ptr(1, 2).* = 0.001 * std.math.exp(-try r.at(0) * try r.at(0));

    U.ptr(2, 0).* = U.at(0, 2);
    U.ptr(2, 1).* = U.at(1, 2);
    U.ptr(2, 2).* = -0.01 * std.math.tanh(0.5 * try r.at(0));
}

pub fn tripleState1D_3(comptime T: type, U: *Matrix(T), r: Vector(T)) !void {
    U.ptr(0, 0).* = 0.03 * (std.math.tanh(1.6 * try r.at(0)) + std.math.tanh(1.6 * (try r.at(0) + 7)));
    U.ptr(0, 1).* = 0.005 * std.math.exp(-try r.at(0) * try r.at(0));
    U.ptr(0, 2).* = 0.005 * std.math.exp(-(try r.at(0) + 7) * (try r.at(0) + 7));

    U.ptr(1, 0).* = U.at(0, 1);
    U.ptr(1, 1).* = -0.03 * (std.math.tanh(1.6 * try r.at(0)) + std.math.tanh(1.6 * (try r.at(0) - 7)));
    U.ptr(1, 2).* = 0.005 * std.math.exp(-(try r.at(0) - 7) * (try r.at(0) - 7));

    U.ptr(2, 0).* = U.at(0, 2);
    U.ptr(2, 1).* = U.at(1, 2);
    U.ptr(2, 2).* = -0.03 * (std.math.tanh(1.6 * (try r.at(0) + 7)) - std.math.tanh(1.6 * (try r.at(0) - 7)));
}

pub fn tully1D_1(comptime T: type, U: *Matrix(T), r: Vector(T)) !void {
    U.ptr(0, 0).* = if (try r.at(0) > 0) 0.01 * (1 - std.math.exp(-1.6 * try r.at(0))) else -0.01 * (1 - std.math.exp(1.6 * try r.at(0)));
    U.ptr(0, 1).* = 0.005 * std.math.exp(-try r.at(0) * try r.at(0));
...
pub fn tully1D_2(comptime T: type, U: *Matrix(T), r: Vector(T)) !void {
    U.ptr(0, 0).* = 0;
    U.ptr(0, 1).* = 0.0150 * std.math.exp(-0.06 * try r.at(0) * try r.at(0));

    U.ptr(1, 0).* = U.at(0, 1);
    U.ptr(1, 1).* = -0.1 * std.math.exp(-0.28 * try r.at(0) * try r.at(0)) + 0.05;
}

pub fn tully1D_3(comptime T: type, U: *Matrix(T), r: Vector(T)) !void {
    U.ptr(0, 0).* = 6e-4;
    U.ptr(0, 1).* = if (try r.at(0) > 0) 0.1 * (2 - std.math.exp(-0.9 * try r.at(0))) else 0.1 * std.math.exp(0.9 * try r.at(0));
...
pub fn uracil1D_1(comptime T: type, U: *Matrix(T), r: Vector(T)) !void {
    ...
    U.ptr(0, 0).* = E_0 / au2ev + 0.5 * (omg_10 * try r.at(0) * try r.at(0) + omg_12 * try r.at(1) * try r.at(1) + omg_18 * try r.at(2) * try r.at(2) + omg_20 * try r.at(3) * try r.at(3) + omg_21 * try r.at(4) * try r.at(4)) / au2cm + ((d_24_0 * (std.math.exp(a_24_0 * (try r.at(5) - q_24_0)) - 1) * (std.math.exp(a_24_0 * (try r.at(5) - q_24_0)) - 1) + e_24_0) + (d_25_0 * (std.math.exp(a_25_0 * (try r.at(6) - q_25_0)) - 1) * (std.math.exp(a_25_0 * (try r.at(6) - q_25_0)) - 1) + e_25_0) + (d_26_0 * (std.math.exp(a_26_0 * (try r.at(7) - q_26_0)) - 1) * (std.math.exp(a_26_0 * (try r.at(7) - q_26_0)) - 1) + e_26_0)) / au2ev + (k_18_0 * try r.at(2) + k_20_0 * try r.at(3) + k_21_0 * try r.at(4)) / au2ev + 0.5 * (g_18_0 * try r.at(2) * try r.at(2) + g_20_0 * try r.at(3) * try r.at(3) + g_21_0 * try r.at(4) * try r.at(4)) / au2ev;
    U.ptr(0, 1).* = (l_10_01 * try r.at(0) + l_12_01 * try r.at(1)) / au2ev;
    U.ptr(0, 2).* = (l_18_02 * try r.at(2) + l_20_02 * try r.at(3) + l_21_02 * try r.at(4) + l_24_02 * try r.at(5) + l_25_02 * try r.at(6) + l_26_02 * try r.at(7)) / au2ev;
    U.ptr(0, 3).* = 0;

    U.ptr(1, 0).* = U.at(0, 1);
    U.ptr(1, 1).* = E_1 / au2ev + 0.5 * (omg_10 * try r.at(0) * try r.at(0) + omg_12 * try r.at(1) * try r.at(1) + omg_18 * try r.at(2) * try r.at(2) + omg_20 * try r.at(3) * try r.at(3) + omg_21 * try r.at(4) * try r.at(4)) / au2cm + ((d_24_1 * (std.math.exp(a_24_1 * (try r.at(5) - q_24_1)) - 1) * (std.math.exp(a_24_1 * (try r.at(5) - q_24_1)) - 1) + e_24_1) + (d_25_1 * (std.math.exp(a_25_1 * (try r.at(6) - q_25_1)) - 1) * (std.math.exp(a_25_1 * (try r.at(6) - q_25_1)) - 1) + e_25_1) + (d_26_1 * (std.math.exp(a_26_1 * (try r.at(7) - q_26_1)) - 1) * (std.math.exp(a_26_1 * (try r.at(7) - q_26_1)) - 1) + e_26_1)) / au2ev + (k_18_1 * try r.at(2) + k_20_1 * try r.at(3) + k_21_1 * try r.at(4)) / au2ev + 0.5 * (g_18_1 * try r.at(2) * try r.at(2) + g_20_1 * try r.at(3) * try r.at(3) + g_21_1 * try r.at(4) * try r.at(4)) / au2ev;
    U.ptr(1, 2).* = (l_10_12 * try r.at(0) + l_12_12 * try r.at(1)) / au2ev;
    U.ptr(1, 3).* = (l_18_13 * try r.at(2) + l_20_13 * try r.at(3) + l_21_13 * try r.at(4) + l_24_13 * try r.at(5) + l_25_13 * try r.at(6) + l_26_13 * try r.at(7)) / au2ev;

    U.ptr(2, 0).* = U.at(0, 2);
    U.ptr(2, 1).* = U.at(1, 2);
    U.ptr(2, 2).* = E_2 / au2ev + 0.5 * (omg_10 * try r.at(0) * try r.at(0) + omg_12 * try r.at(1) * try r.at(1) + omg_18 * try r.at(2) * try r.at(2) + omg_20 * try r.at(3) * try r.at(3) + omg_21 * try r.at(4) * try r.at(4)) / au2cm + ((d_24_2 * (std.math.exp(a_24_2 * (try r.at(5) - q_24_2)) - 1) * (std.math.exp(a_24_2 * (try r.at(5) - q_24_2)) - 1) + e_24_2) + (d_25_2 * (std.math.exp(a_25_2 * (try r.at(6) - q_25_2)) - 1) * (std.math.exp(a_25_2 * (try r.at(6) - q_25_2)) - 1) + e_25_2) + (d_26_2 * (std.math.exp(a_26_2 * (try r.at(7) - q_26_2)) - 1) * (std.math.exp(a_26_2 * (try r.at(7) - q_26_2)) - 1) + e_26_2)) / au2ev + (k_18_2 * try r.at(2) + k_20_2 * try r.at(3) + k_21_2 * try r.at(4)) / au2ev + 0.5 * (g_18_2 * try r.at(2) * try r.at(2) + g_20_2 * try r.at(3) * try r.at(3) + g_21_2 * try r.at(4) * try r.at(4)) / au2ev;
    U.ptr(2, 3).* = 0;

    U.ptr(3, 0).* = U.at(0, 3);
    U.ptr(3, 1).* = U.at(1, 3);
    U.ptr(3, 2).* = U.at(2, 3);
    U.ptr(3, 3).* = E_3 / au2ev + 0.5 * (omg_10 * try r.at(0) * try r.at(0) + omg_12 * try r.at(1) * try r.at(1) + omg_18 * try r.at(2) * try r.at(2) + omg_20 * try r.at(3) * try r.at(3) + omg_21 * try r.at(4) * try r.at(4)) / au2cm + ((d_24_3 * (std.math.exp(a_24_3 * (try r.at(5) - q_24_3)) - 1) * (std.math.exp(a_24_3 * (try r.at(5) - q_24_3)) - 1) + e_24_3) + (d_25_3 * (std.math.exp(a_25_3 * (try r.at(6) - q_25_3)) - 1) * (std.math.exp(a_25_3 * (try r.at(6) - q_25_3)) - 1) + e_25_3) + (d_26_3 * (std.math.exp(a_26_3 * (try r.at(7) - q_26_3)) - 1) * (std.math.exp(a_26_3 * (try r.at(7) - q_26_3)) - 1) + e_26_3)) / au2ev + (k_18_3 * try r.at(2) + k_20_3 * try r.at(3) + k_21_3 * try r.at(4)) / au2ev + 0.5 * (g_18_3 * try r.at(2) * try r.at(2) + g_20_3 * try r.at(3) * try r.at(3) + g_21_3 * try r.at(4) * try r.at(4)) / au2ev;
}
    ...
    for (0..R.rows) |i| {
        ...
        for (0..ndim) |j| {
            ...
            for (0..opt.constant.len) |k| if (opt.constant[k].index == j) {
                (try r.ptr(j)).* = opt.constant[k].value; constant = true;
            };

            if (!constant) {
                (try r.ptr(j)).* = R.at(i, l); l += 1;
            }
        }
