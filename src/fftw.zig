...
pub fn fftwnd(in: []Complex(f64), shape: []const i32, factor: i32) void {
    const plan = fftw.fftw_plan_dft(@intCast(shape.len), &shape[0], &@as([][2]f64, @ptrCast(in))[0], &@as([][2]f64, @ptrCast(in))[0], factor, fftw.FFTW_ESTIMATE);
