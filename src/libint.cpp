...
extern "C" {
                ...
                for (size_t n = 0; n < shells.at(j).size(); n++) {
                    for (size_t o = 0; o < shells.at(k).size(); o++) {
                        for (size_t p = 0; p < shells.at(l).size(); p++) {
                            ...
                            F[bf1 * nbf + bf2] += 0.500 * D[bf3 * nbf + bf4] * engines.at(id).results().at(0)[integral_index  ] * degeneracy;
                            F[bf3 * nbf + bf4] += 0.500 * D[bf1 * nbf + bf2] * engines.at(id).results().at(0)[integral_index  ] * degeneracy;
                            F[bf1 * nbf + bf3] -= 0.125 * D[bf2 * nbf + bf4] * engines.at(id).results().at(0)[integral_index  ] * degeneracy;
                            F[bf2 * nbf + bf4] -= 0.125 * D[bf1 * nbf + bf3] * engines.at(id).results().at(0)[integral_index  ] * degeneracy;
                            F[bf1 * nbf + bf4] -= 0.125 * D[bf2 * nbf + bf3] * engines.at(id).results().at(0)[integral_index  ] * degeneracy;
                            F[bf2 * nbf + bf3] -= 0.125 * D[bf1 * nbf + bf4] * engines.at(id).results().at(0)[integral_index++] * degeneracy;
                        }
                    }
                }
            }
    ...
    void fock(double *F, ulong natoms, const double *anums, const double *coords, ulong nbasis, const double *basis, const double *D) {
        auto [atoms, shells] = load(natoms, anums, coords, nbasis, basis); int max_l = 0; size_t max_npgs = 0;
        ...
        twoelec2fock(F, engine, shells, D); libint2::finalize();
    }
}
