...
template<bool G>
void twoelec2fock(double *F, libint2::Engine &engine, const std::vector<libint2::Shell> &shells, const double *D) {
    int nbf = 0; std::vector<size_t> sh2bf; std::vector<libint2::Engine> engines(std::getenv("OMP_NUM_THREADS") ? std::max(1, std::atoi(std::getenv("OMP_NUM_THREADS"))) : 1, engine);

    for (int i = 0, j = 0; j < shells.size(); i += shells[j].size(), j++) sh2bf.push_back(i);

    for (const auto& shell : shells) nbf += (G ? 2 : 1) * shell.size();

    std::vector<std::vector<double>> Fs(engines.size(), std::vector<double>(nbf * nbf, 0.0));
                        ...
                        if constexpr (G) {
                            for (int s = 0; s < 2; s++) {
                                for (int t = 0; t < 2; t++) {
                                    ...
                                    int sbf1 = s * nbf / 2 + bf1; int sbf2 = s * nbf / 2 + bf2; int sbf3 = t * nbf / 2 + bf3; int sbf4 = t * nbf / 2 + bf4;

                                    Fs.at(id).at(sbf1 * nbf + sbf2) += 0.5 * D[sbf3 * nbf + sbf4] * value * degeneracy;
                                    Fs.at(id).at(sbf3 * nbf + sbf4) += 0.5 * D[sbf1 * nbf + sbf2] * value * degeneracy;

                                    if (s == t) {
                                        Fs.at(id).at(sbf1 * nbf + sbf3) -= 0.25 * D[sbf2 * nbf + sbf4] * value * degeneracy;
                                        Fs.at(id).at(sbf2 * nbf + sbf4) -= 0.25 * D[sbf1 * nbf + sbf3] * value * degeneracy;
                                        Fs.at(id).at(sbf1 * nbf + sbf4) -= 0.25 * D[sbf2 * nbf + sbf3] * value * degeneracy;
                                        Fs.at(id).at(sbf2 * nbf + sbf3) -= 0.25 * D[sbf1 * nbf + sbf4] * value * degeneracy;
                                    }
                                }
                            }
                        } else {
    ...
    for (size_t i = 0; i < Fs.size(); i++) for (size_t j = 0; j < nbf * nbf; j++) {
        F[j] += Fs.at(i).at(j);
    }
...
extern "C" {
    ...
    void fock(double *F, ulong natoms, const double *anums, const double *coords, ulong nbasis, const double *basis, const double *D, bool G) {
        ...
        G ? twoelec2fock<true>(F, engine, shells, D) : twoelec2fock<false>(F, engine, shells, D); libint2::finalize();
    }
}
