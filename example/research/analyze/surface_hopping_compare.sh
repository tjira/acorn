...
for MODEL in ${MODELS[@]}; do
    ...
    for MOMENTUM in ${MOMENTA[@]}; do
        ...
        if [[ $NSTATE -eq 2 ]]; then
            ...
            $PLOT "POTENTIAL_ENERGY_${MODEL}_P=${MOMENTUM}_EXACT.mat" "POTENTIAL_ENERGY_MEAN_${MODEL}_P=${MOMENTUM}_FSSH.mat" "POTENTIAL_ENERGY_MEAN_${MODEL}_P=${MOMENTUM}_LZSH.mat" --title "POTENTIAL ENERGY: ${MODEL}"     --xlabel "Time (a.u.)" --ylabel "Energy (a.u.)"     --legend "EXACT" "FSSH" "LZSH" --output "POTENTIAL_ENERGY_${MODEL}_P=${MOMENTUM}" --png
            $PLOT "TOTAL_ENERGY_${MODEL}_P=${MOMENTUM}_EXACT.mat"     "TOTAL_ENERGY_MEAN_${MODEL}_P=${MOMENTUM}_FSSH.mat"     "TOTAL_ENERGY_MEAN_${MODEL}_P=${MOMENTUM}_LZSH.mat"     --title "TOTAL ENERGY: ${MODEL}"         --xlabel "Time (a.u.)" --ylabel "Energy (a.u.)"     --legend "EXACT" "FSSH" "LZSH" --output "TOTAL_ENERGY_${MODEL}_P=${MOMENTUM}"     --png
