...
pub fn expect(val: anytype, ref: @TypeOf(val)) !void {
    std.testing.expect(@abs(val - ref) < 1e-12) catch |err| {
        std.debug.print("VALUE: {d:20.14}, REFERENCE: {d:20.14}\n", .{val, ref}); return err;
    };
}
...
comptime {
    _ = classical_dynamics; _ = configuration_interaction; _ = hartree_fock; _ = matrix; _ = moller_plesset; _ = quantum_dynamics; _ = tensor; _ = vector;
}
