...
# EXAMPLES
# ./neqdet.py -n 2 -w "[np.exp(-(r1-1)**2-(r2-1)**2)]" -v "[[0.5*(r1**2+r2**2)]]" --real
# ./neqdet.py -d 0 -f 0.01 -g 2048 -l 32 -m 2000 -t 10 -v "[[0.01*np.tanh(0.6*r1),0.001*np.exp(-r1**2)],[0.001*np.exp(-r1**2),-0.01*np.tanh(0.6*r1)]]" -w "[0,np.exp(-(r1+10)**2+10j*r1)]" --align --real
...
if __name__ == "__main__":
    ...
    for i in range(args.states):
        ...
        for j in range(args.iterations + 1):
            ...
            if (j): psi = np.fft.fftn(psi.reshape(args.dimension * [args.points] + [psi.shape[1]]), axes=range(args.dimension)).reshape(psi.shape)

            # propagate in momentum space
            if (j): psi = np.einsum("ijk,ik->ij", K, psi)

            # inverse fourier transform the wavefunction
            if (j): psi = np.fft.ifftn(psi.reshape(args.dimension * [args.points] + [psi.shape[1]]), axes=range(args.dimension)).reshape(psi.shape)
            ...
            if not args.real or j == 0: psi /= np.sqrt(dr * np.einsum("ij,ij->", psi.conj(), psi))
            ...
            psik = np.fft.fftn(psi.reshape(args.dimension * [args.points] + [psi.shape[1]]), axes=range(args.dimension)).reshape(psi.shape)
            ...
            Ekin = np.sum(psi.conj() * np.fft.ifftn(psik.reshape(args.dimension * [args.points] + [psi.shape[1]]), axes=range(args.dimension)).reshape(psi.shape)).real * dr
    ...
    if args.dimension == 1: wfnplot = np.array([[axs[0, 0].plot(r, wfn[0][:, i].real)[0], axs[0, 0].plot(r, wfn[0][:, i].imag)[0]] for i in range(psi.shape[1])]).flatten()
    ...
    if args.dimension == 1: ani = anm.FuncAnimation(fig, update, frames=range(len(wfn)), repeat=True, interval=30)
