...
for POP_H in $(ls POPULATION_H_2D_E=*.mat); do

    E=$(echo "$POP_H" | sed 's/POPULATION_H_2D_E=// ; s/.mat//')
    ...
    if [[ ! -z "$PROB_H" && ! -z "$PROB_D" && ! -z "$PROB_T" ]]; then
        echo "$E $PROB_H $PROB_D $PROB_T" >> PROBABILITY_EXACT_2D.mat
    fi
done

sed -i "1i $(wc -l < PROBABILITY_EXACT_1D.mat) 4" PROBABILITY_EXACT_1D.mat
sed -i "1i $(wc -l < PROBABILITY_EXACT_2D.mat) 4" PROBABILITY_EXACT_2D.mat

matsort -i PROBABILITY_EXACT_1D.mat -o PROBABILITY_EXACT_1D.mat
matsort -i PROBABILITY_EXACT_2D.mat -o PROBABILITY_EXACT_2D.mat
...
lines.py PROBABILITY_EXACT_1D.mat PROBABILITY_EXACT_2D.mat --alphas 0,1,2 0.7 0.7 0.7 --colors 3,4,5 "darkblue" "darkorange" "darkgreen" --legends every "H (1D)" "D (1D)" "T (1D)" "H (2D)" "D (2D)" "T (2D)" --legpos 0.3 1 --styles 3,4,5 dashed dashed dashed --xlabel "Kinetic Energy (a.u.)" --xlim 0.035 2 --ylabel "Charge Transfer Probability" --ylim 0 0.01 --output PROBABILITY_COMPARE_DIM.png
