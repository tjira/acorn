...
for MODEL in ${MODELS[@]}; do
    ...
    for MOMENTUM in ${MOMENTA[@]}; do
        ...
        IS_COL_FSSH=$( head -n 2  "POPULATION_${MODEL}_P=${MOMENTUM}_FSSH.mat" | tail -n 1 | awk '{is=2; pop=$2; for(i=3; i<=NF; i++) if ($i > pop) {is=i; pop=$i;} print is - 2}')
        IS_COL_KFSSH=$(head -n 2 "POPULATION_${MODEL}_P=${MOMENTUM}_KFSSH.mat" | tail -n 1 | awk '{is=2; pop=$2; for(i=3; i<=NF; i++) if ($i > pop) {is=i; pop=$i;} print is - 2}')
        IS_COL_LZSH=$( head -n 2  "POPULATION_${MODEL}_P=${MOMENTUM}_LZSH.mat" | tail -n 1 | awk '{is=2; pop=$2; for(i=3; i<=NF; i++) if ($i > pop) {is=i; pop=$i;} print is - 2}')

        POP_EXACT=$(tail -n 1 "POPULATION_${MODEL}_P=${MOMENTUM}_EXACT.mat" | awk -v i=$IS_COL_EXACT '{print $(i+1)}');
        POP_FSSH=$( tail -n 1  "POPULATION_${MODEL}_P=${MOMENTUM}_FSSH.mat" | awk -v i=$IS_COL_FSSH  '{print $(i+1)}');
        POP_KFSSH=$(tail -n 1 "POPULATION_${MODEL}_P=${MOMENTUM}_KFSSH.mat" | awk -v i=$IS_COL_KFSSH '{print $(i+1)}');
        POP_LZSH=$( tail -n 1  "POPULATION_${MODEL}_P=${MOMENTUM}_LZSH.mat" | awk -v i=$IS_COL_LZSH  '{print $(i+1)}');

        echo "${MOMENTUM} ${POP_EXACT} ${POP_FSSH} ${POP_KFSSH} ${POP_LZSH}" >> "FINAL_POPULATION_P_${MODEL}.mat"

        $PLOT_1D "POPULATION_${MODEL}_P=${MOMENTUM}_EXACT.mat" "POPULATION_${MODEL}_P=${MOMENTUM}_FSSH.mat" "POPULATION_${MODEL}_P=${MOMENTUM}_KFSSH.mat" "POPULATION_${MODEL}_P=${MOMENTUM}_LZSH.mat" --title "ADIABATIC POPULATION: ${MODEL}" --xlabel "Time (a.u.)" --ylabel "State Population" --legend "${LEGEND_POP[@]}" --output "POPULATION_${MODEL}_P=${MOMENTUM}" --png

        montage "POTENTIAL_ADIABATIC_${MODEL}.png" "POPULATION_${MODEL}_P=${MOMENTUM}.png" -mode concatenate -tile x1 "TRAJECTORY_GENERAL_${MODEL}_P=${MOMENTUM}.png"
    done
    ...
    montage "POTENTIAL_DIABATIC_${MODEL}.png" "FINAL_POPULATION_P_${MODEL}.png"  -mode concatenate -tile x1 "FINAL_POPULATION_${MODEL}.png"
done
