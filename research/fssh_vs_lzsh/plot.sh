...
PLOT_1D=./../../python/plot.py; MODELS=($(ls -l exact* | grep -oP "(?<=exact_)[^_]+_[^_]+" | sort | uniq)); MOMENTA=($(ls -l exact* | grep -oP "P=\K[0-9.]+" | sed 's/.$//' | sort | uniq))
...
for MODEL in ${MODELS[@]}; do
    ...
    for MOMENTUM in ${MOMENTA[@]}; do
        ...
        IS_COL_MASH=$( head -n 2 "POPULATION_MEAN_${MODEL}_P=${MOMENTUM}_MASH.mat"  | tail -n 1 | awk '{is=2; pop=$2; for(i=3; i<=NF; i++) if ($i > pop) {is=i; pop=$i;} print is - 2}')
        ...
        POP_MASH=$( tail -n 1 "POPULATION_MEAN_${MODEL}_P=${MOMENTUM}_MASH.mat"  | awk -v i=$IS_COL_LZSH  '{print $(i+2)}');

        if [[ $NSTATE -eq 2 ]]; then
            $PLOT_1D "POPULATION_${MODEL}_P=${MOMENTUM}_EXACT.mat"       "POPULATION_MEAN_${MODEL}_P=${MOMENTUM}_FSSH.mat"       "POPULATION_MEAN_${MODEL}_P=${MOMENTUM}_LZSH.mat"       "POPULATION_MEAN_${MODEL}_P=${MOMENTUM}_MASH.mat"       --title "ADIABATIC POPULATION: ${MODEL}" --xlabel "Time (a.u.)" --ylabel "State Population"  --legend "${LEGEND_POP[@]}"           --output "POPULATION_${MODEL}_P=${MOMENTUM}"       --png
            $PLOT_1D "KINETIC_ENERGY_${MODEL}_P=${MOMENTUM}_EXACT.mat"   "KINETIC_ENERGY_MEAN_${MODEL}_P=${MOMENTUM}_FSSH.mat"   "KINETIC_ENERGY_MEAN_${MODEL}_P=${MOMENTUM}_LZSH.mat"   "KINETIC_ENERGY_MEAN_${MODEL}_P=${MOMENTUM}_MASH.mat"   --title "KINETIC ENERGY: ${MODEL}"       --xlabel "Time (a.u.)" --ylabel "Energy (a.u.)"     --legend "EXACT" "FSSH" "LZSH" "MASH" --output "KINETIC_ENERGY_${MODEL}_P=${MOMENTUM}"   --png
            ...
            $PLOT_1D "TOTAL_ENERGY_${MODEL}_P=${MOMENTUM}_EXACT.mat"     "TOTAL_ENERGY_MEAN_${MODEL}_P=${MOMENTUM}_FSSH.mat"     "TOTAL_ENERGY_MEAN_${MODEL}_P=${MOMENTUM}_LZSH.mat"     "TOTAL_ENERGY_MEAN_${MODEL}_P=${MOMENTUM}_MASH.mat"     --title "TOTAL ENERGY: ${MODEL}"         --xlabel "Time (a.u.)" --ylabel "Energy (a.u.)"     --legend "EXACT" "FSSH" "LZSH" "MASH" --output "TOTAL_ENERGY_${MODEL}_P=${MOMENTUM}"     --png
            $PLOT_1D "POSITION_${MODEL}_P=${MOMENTUM}_EXACT.mat"         "POSITION_MEAN_${MODEL}_P=${MOMENTUM}_FSSH.mat"         "POSITION_MEAN_${MODEL}_P=${MOMENTUM}_LZSH.mat"         "POSITION_MEAN_${MODEL}_P=${MOMENTUM}_MASH.mat"         --title "POSITION: ${MODEL}"             --xlabel "Time (a.u.)" --ylabel "Coordinate (a.u.)" --legend "EXACT" "FSSH" "LZSH" "MASH" --output "POSITION_${MODEL}_P=${MOMENTUM}"         --png
            $PLOT_1D "MOMENTUM_${MODEL}_P=${MOMENTUM}_EXACT.mat"         "MOMENTUM_MEAN_${MODEL}_P=${MOMENTUM}_FSSH.mat"         "MOMENTUM_MEAN_${MODEL}_P=${MOMENTUM}_LZSH.mat"         "MOMENTUM_MEAN_${MODEL}_P=${MOMENTUM}_MASH.mat"         --title "MOMENTUM: ${MODEL}"             --xlabel "Time (a.u.)" --ylabel "Momentum (a.u.)"   --legend "EXACT" "FSSH" "LZSH" "MASH" --output "MOMENTUM_${MODEL}_P=${MOMENTUM}"         --png

            echo "${MOMENTUM} ${POP_EXACT} ${POP_FSSH} ${POP_KFSSH} ${POP_LZSH} ${POP_MASH}" >> "FINAL_POPULATION_P_${MODEL}.mat"
        else
            $PLOT_1D "POPULATION_${MODEL}_P=${MOMENTUM}_EXACT.mat"       "POPULATION_MEAN_${MODEL}_P=${MOMENTUM}_FSSH.mat"       "POPULATION_MEAN_${MODEL}_P=${MOMENTUM}_LZSH.mat"       --title "ADIABATIC POPULATION: ${MODEL}" --xlabel "Time (a.u.)" --ylabel "State Population"  --legend "${LEGEND_POP[@]}"    --output "POPULATION_${MODEL}_P=${MOMENTUM}"       --png
            $PLOT_1D "KINETIC_ENERGY_${MODEL}_P=${MOMENTUM}_EXACT.mat"   "KINETIC_ENERGY_MEAN_${MODEL}_P=${MOMENTUM}_FSSH.mat"   "KINETIC_ENERGY_MEAN_${MODEL}_P=${MOMENTUM}_LZSH.mat"   --title "KINETIC ENERGY: ${MODEL}"       --xlabel "Time (a.u.)" --ylabel "Energy (a.u.)"     --legend "EXACT" "FSSH" "LZSH" --output "KINETIC_ENERGY_${MODEL}_P=${MOMENTUM}"   --png
            $PLOT_1D "POTENTIAL_ENERGY_${MODEL}_P=${MOMENTUM}_EXACT.mat" "POTENTIAL_ENERGY_MEAN_${MODEL}_P=${MOMENTUM}_FSSH.mat" "POTENTIAL_ENERGY_MEAN_${MODEL}_P=${MOMENTUM}_LZSH.mat" --title "POTENTIAL ENERGY: ${MODEL}"     --xlabel "Time (a.u.)" --ylabel "Energy (a.u.)"     --legend "EXACT" "FSSH" "LZSH" --output "POTENTIAL_ENERGY_${MODEL}_P=${MOMENTUM}" --png
            $PLOT_1D "TOTAL_ENERGY_${MODEL}_P=${MOMENTUM}_EXACT.mat"     "TOTAL_ENERGY_MEAN_${MODEL}_P=${MOMENTUM}_FSSH.mat"     "TOTAL_ENERGY_MEAN_${MODEL}_P=${MOMENTUM}_LZSH.mat"     --title "TOTAL ENERGY: ${MODEL}"         --xlabel "Time (a.u.)" --ylabel "Energy (a.u.)"     --legend "EXACT" "FSSH" "LZSH" --output "TOTAL_ENERGY_${MODEL}_P=${MOMENTUM}"     --png
