...
do for [i=2:columns*frames+1:columns] {
    plot for [j=1:ARGC] for [k=1:words(only)] ARGV[j] u 1:(word(scale, (word(only, k)-1)%words(scale)+1)*column(i+word(only, k)-1)+word(shift, (word(only, k)-1)%words(shift)+1)) w li s u not, for [i=2:*] background u 1:i w li s u not
}
