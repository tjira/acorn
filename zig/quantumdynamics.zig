...
pub fn run(comptime T: type, opt: QuantumDynamicsOptions(T), allocator: std.mem.Allocator) !void {
    ...
    {
        var r = try Matrix(T).init(std.math.pow(u32, opt.grid.points, ndim), ndim, allocator); defer r.deinit();
        var k = try Matrix(T).init(std.math.pow(u32, opt.grid.points, ndim), ndim, allocator); defer k.deinit();
        ...
        mpt.rgrid(T, &r, opt.grid.limits[0], opt.grid.limits[1], opt.grid.points);
        mpt.kgrid(T, &k, opt.grid.limits[0], opt.grid.limits[1], opt.grid.points);

        var R = try rgridPropagators(T, opt, r, allocator); defer R.deinit();
        var K = try kgridPropagators(T, opt, k, allocator); defer K.deinit();
        var V = try rgridPotentials (T, opt, r, allocator); defer V.deinit();

        wfnGuess(T, &W, r, opt.initial_conditions.position, opt.initial_conditions.momentum, opt.initial_conditions.state); const dr = r.at(1, 0) - r.at(0, 0);

        for (0..opt.iterations) |i| {

            wfnPropagate(T, &W, R, K, &T1);

            const norm = wfnNorm(T, W, r.at(1, 0) - r.at(0, 0)); for (0..W.rows) |j| W.ptr(j, 0).* = W.at(j, 0).div(std.math.Complex(T).init(norm, 0));

            const Ekin = wfnKineticEnergy(T, W, k, opt.initial_conditions.mass, dr, &T1, &T2); const Epot: T = wfnPotentialEnergy(T, W, V, dr);

            wfnDensity(T, &P, W, dr); for (0..nstate) |j| pop.ptr(i, j).* = P.at(j, j);

            try stdout.print("{d:6} {d:12.6} {d:12.6} {d:12.6} {d:4.3}\n", .{i + 1, Ekin, Epot, Ekin + Epot, P.at(0, 0)});
        }
...
pub fn wfnGuess(comptime T: type, W: *Matrix(std.math.Complex(T)), rvec: Matrix(T), position: []const T, momentum: []const T, state: u32) void {
    ...
    const norm = wfnNorm(T, W.*, rvec.at(1, 0) - rvec.at(0, 0)); for (0..W.rows) |j| W.ptr(j, 0).* = W.at(j, 0).div(std.math.Complex(T).init(norm, 0));
}

pub fn wfnKineticEnergy(comptime T: type, W: Matrix(std.math.Complex(T)), kvec: Matrix(T), mass: T, dr: T, T1: *Matrix(std.math.Complex(T)), T2: *Matrix(std.math.Complex(T))) T {
    var Ekin: T = 0;

    ftr.dft(T, T1.data, W.data, &[_]usize{W.rows}, -1);
    for (0..W.rows) |j| T1.ptr(j, 0).* = T1.at(j, 0).mul(std.math.Complex(T).init(kvec.at(j, 0) * kvec.at(j, 0), 0));
    ftr.dft(T, T2.data, T1.data, &[_]usize{W.rows},  1);
    for (0..W.rows) |j| Ekin += (T2.at(j, 0).mul(std.math.complex.conj(W.at(j, 0)))).re;
...
pub fn wfnNorm(comptime T: type, W: Matrix(std.math.Complex(T)), dr: T) T {
    ...
    for (0..W.rows) |i| norm = norm.add(std.math.complex.conj(W.at(i, 0)).mul(W.at(i, 0)));
...
pub fn wfnPotentialEnergy(comptime T: type, W: Matrix(std.math.Complex(T)), V: std.ArrayList(Matrix(std.math.Complex(T))), dr: T) T {
    ...
    for (0..W.rows) |j| {
        Epot += std.math.complex.abs(std.math.complex.conj(W.at(j, 0)).mul(V.items[j].at(0, 0).mul(W.at(j, 0))));
    }

    return Epot * dr;
}

pub fn wfnPropagate(comptime T: type, W: *Matrix(std.math.Complex(T)), R: std.ArrayList(Matrix(std.math.Complex(T))), K: @TypeOf(R), T1: @TypeOf(W)) void {
